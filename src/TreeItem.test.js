...
test('basicTreeItemTest', () => {
    ...
    ti.addLogEntry(logEntry1);
    expect(ti.getParent()).toBeDefined();
    expect(ti.getChildItems().length).toBe(0);

    ti.addLogEntry(logEntry2);
    ti.addLogEntry(logEntry3);
    ti.addLogEntry(logEntry4);
...
test('treeItemNonMatchingLogGroupId', () => {
    ...
    let ti = new TreeItem();
    ti.addLogEntry(logEntry1);
    ti.addLogEntry(logEntry2); // Will be added as group id does not match.
...
test('treeItemNullParentGroupId', () => {
    ...
    let ti = new TreeItem();
    ti.addLogEntry(logEntry1);
    ti.addLogEntry(logEntry2); // Will be added as parent does not define group id.
