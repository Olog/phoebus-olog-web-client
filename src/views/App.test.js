...
describe("Search Results", () => {
  ...
  it("allows user to manually enter search terms", async () => {
    ...
    givenServerRespondsWithSearchRequest({
      ...
      requestPredicate: (req) =>
        req.url.searchParams.get("start") === "987654321"
    });
  ...
  it("user can search by toggling the filter area", async () => {
    ...
    givenServerRespondsWithSearchRequest({
      title: "hmmm title",
      requestPredicate: (req) =>
        req.url.searchParams.get("title") === "some value"
    });

    // Open the filters
    const showAdvancedSearchButton = await screen.findByRole("button", {
    ...
    // Close the filters
    const hideAdvancedSearchButton = await screen.findByRole("button", {
      name: /hide advanced search/i
    });
  ...
  it("updates search results instantly from the search filter bar for logbooks", async () => {
    ...
    givenServerRespondsWithSearchRequest({
      title: "hmmm title",
      requestPredicate: (req) =>
        req.url.searchParams.get("logbooks") === "test controls"
    });
  ...
  test("user can view a log entry directly by ID, even if it is not in the search results", async () => {
    ...
    server.use(
      rest.get(`*/logs/${id}`, (req, res, ctx) => {
        return res(ctx.json(testEntry({ title, id })));
      })
    );
  ...
  test("user cannot navigate directly to a nonexistent log", async () => {
    ...
    // Then we expect to find that entry on the page
    const notFoundMessage = await screen.findByRole("heading", {
      name: /log record .* not found/i
    });
...
describe("Login/Logout", () => {
  ...
  it("displays username and allows creating log entries when signed in", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /garfieldHatesMondays/i })
    ).toBeInTheDocument();
  });
  ...
  test("when navigating to create a log entry directly but not logged in, the user is prompted to login", async () => {
    ...
    // when rendered
    render(<TestRouteProvider initialEntries={["/logs/create"]} />);
...
describe("Creating Log Entries", () => {
  test("when an user clicks on a group entry and it is already in the search results, that log entry is displayed", async () => {
    ...
    const entry1 = {
      ...testEntry({ title: "grouped entry 1", id: 1 }),
      properties: [
        {
    ...
    const entry2 = {
      ...testEntry({ title: "grouped entry 2", id: 2 }),
      properties: [
        {
          name: "Log Entry Group",
          owner: null,
    ...
    server.use(
      rest.get("*/logs/search", (req, res, ctx) => {
        return res(ctx.json(resultList([entry1, entry2])));
      }),
      rest.get("*/logs", (req, res, ctx) => {
        if (
          req.url.searchParams.get("properties") ===
          "Log Entry Group.id.50480e12-cfd3-400d-a4db-2045cba08901"
        ) {
          return res(ctx.json([entry1, entry2]));
        }
      })
    );
    ...
    const entry1Result = await screen.findByRole("heading", {
      name: entry1.title
    });
    await user.click(entry1Result);
    const showGroupButton = screen.getByRole("button", {
      name: /show.*group/i
    });
    await user.click(showGroupButton);
    const groupEntriesElement = await screen.findByRole("list", {
      name: /group entries/i
    });
    ...
    const entry2Title = screen.getByRole("heading", {
      name: entry2.title,
      level: 2
    }); // we should NOT need to await this
  ...
  test("when an user clicks on a group entry and it is NOT already in the search results, that log entry is still displayed", async () => {
    ...
    server.use(
      rest.get("*/logs/search", (req, res, ctx) => {
        return res(ctx.json(resultList([entry1])));
      }),
      rest.get("*/logs", (req, res, ctx) => {
        if (
          req.url.searchParams.get("properties") ===
    ...
    // Then that entry is displayed
    const entry2Title = await screen.findByRole("heading", {
      name: entry2.title,
      level: 2
    }); // we SHOULD await this since we expect a network request to display it
    expect(entry2Title).toBeInTheDocument();
