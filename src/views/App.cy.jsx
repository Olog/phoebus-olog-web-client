...
describe("Default Behaviors", () => {
  it("renders with a default search query", () => {
    ...
    cy.findByRole("searchbox", { name: /search logs/i }).should(
      "have.value",
      "start=12 hours&end=now"
    );
  });
...
describe("Search Interface", () => {
  it("supports searching with the search box", () => {
    ...
    const start = uuidV4();
    cy.intercept("GET", "**/logs/search*", (req) => {
      req.reply({
        statusCode: 200,
        body: req.query.start === start ? resultList([entry]) : resultList([])
      });
    }).as("search");

    // When searched
    cy.findByRole("searchbox", { name: /search logs/i }).clear();
    cy.findByRole("searchbox", { name: /search logs/i }).type(
      `start=${start}{Enter}`
    );
  ...
  it("supports search by toggling advanced search", () => {
    ...
    cy.intercept("GET", "**/logs/search*", (req) => {
      req.reply({
        statusCode: 200,
        body:
          req.query.title === entry.title ? resultList([entry]) : resultList([])
      });
    }).as("search");
  ...
  it.skip("searches tags instantly from advanced search", () => {
    ...
    cy.intercept("GET", "**/logs/search*", (req) => {
      req.reply({
        statusCode: 200,
        body: req.query.tags === "foo" ? resultList([entry]) : resultList([])
      });
    }).as("search");
    ...
    cy.findByRole("heading", { name: /advanced search/i }).should("exist");
    cy.wait("@tags");
    cy.findByLabelText(/tags/i)
      .should("exist")
      .type("foo{downArrow}{enter}", { force: true });
  ...
  it.skip("searches logbooks instantly from advanced search", () => {
    ...
    cy.intercept("GET", "**/logs/search*", (req) => {
      req.reply({
        statusCode: 200,
        body:
          req.query.logbooks === "test controls"
            ? resultList([entry])
            : resultList([])
      });
    }).as("search");
    ...
    cy.findByRole("heading", { name: /advanced search/i }).should("exist");
    cy.wait("@logbooks");
    cy.findByLabelText(/logbooks/i)
      .should("exist")
      .type("test controls{downArrow}{enter}", { force: true });
  ...
  it.skip("executes the same query repeatedly (no caching)", () => {
    ...
    cy.findByRole("searchbox", { name: /Search/i })
      .should("exist")
      .click();
    cy.findByRole("searchbox", { name: /Search/i }).type("{Enter}", {
      force: true
    });
    cy.wait("@search1");
...
describe("Navigating Results", () => {
  it("can navigate different log entries by clicking on the next/previous buttons on the log entry", () => {
    ...
    cy.intercept("GET", `**/logs/${entry1.id}`, {
      statusCode: 200,
      body: entry1
    }).as("log1");
    cy.intercept("GET", `**/logs/${entry2.id}`, {
      statusCode: 200,
      body: entry2
    }).as("log2");
    cy.intercept("GET", `**/logs/${entry3.id}`, {
      statusCode: 200,
      body: entry3
    }).as("log3");
  ...
  it("can view a log directly, even when not in search results list", () => {
    ...
    cy.wait("@search");
    cy.wait("@log1");
    cy.findByRole("heading", { name: testEntry.title, level: 2 }).should(
      "exist"
    );
  });
  ...
  it("cannot view a log directly if it doesn't exist", () => {
    ...
    cy.wait("@notfound");
    cy.findByRole("heading", { name: /log record .* not found/i }).should(
      "exist"
    );
  });
});
