...
test('when user submits search many times, search is performed many times (no caching)', async () => {
    ...
    const { unmount } = render(
        <MemoryRouter>
            <LogEntriesView logbooks={[]} tags={[]} />
        </MemoryRouter>
    )

    // Given the page has performed the initial search request
    const initialResult = await screen.findByRole('heading', {name: /example title/i});
    ...
    expect(await screen.findByRole('heading', {name: entry1.title})).toBeInTheDocument();
    expect(screen.queryByRole('heading', {name: entry2.title})).not.toBeInTheDocument();
    expect(screen.queryByRole('heading', {name: entry3.title})).not.toBeInTheDocument();

    // When they submit again, we find the second result set
    await user.keyboard('{Enter}');
    expect(await screen.findByRole('heading', {name: entry1.title})).toBeInTheDocument();
    expect(await screen.findByRole('heading', {name: entry2.title})).toBeInTheDocument();
    expect(screen.queryByRole('heading', {name: entry3.title})).not.toBeInTheDocument();
    ...
    expect(await screen.findByRole('heading', {name: entry1.title})).toBeInTheDocument();
    expect(await screen.findByRole('heading', {name: entry2.title})).toBeInTheDocument();
    expect(await screen.findByRole('heading', {name: entry3.title})).toBeInTheDocument();
