...
const CustomTitle = ({title}) => {
    return (
        <Stack flexDirection="row" gap={1} alignItems="center" >
            ...
            <Typography variant="button" verticalalign="center">{title}</Typography>
        </Stack>
    )
}
...
const CustomContent = ({content}) => {
    return (
        <Typography fontFamily="monospace">{content}</Typography>
    )
}

export const Default = ({title, titleIsCustomComponent, content, contentIsCustomComponent, ...props}) => {
    ...
    return (
        <>
            ...
            <Modal 
                ...
                content={contentIsCustomComponent ? <CustomContent content={content} /> : content }
                {...props}
            />
        </>
    )
